...
{
	public class SceneManagement : ISceneManager
	{
		private readonly ScenesPreset _scenes;

		public SceneManagement(ScenesPreset scenes)
		{
			_scenes = scenes;
		}

		void ISceneManager.LoadSceneByRef(SceneType sceneType, bool additive, Action<SceneAsset> callback) => LoadScene(GetSceneByType(sceneType), additive, callback);

		void ISceneManager.UnloadSceneByRef(SceneAsset sceneRef, Action callback) => UnloadSceneByRef(sceneRef, callback);
