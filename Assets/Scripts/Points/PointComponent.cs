...
namespace BurrgerCity.Gameplay.Points
{
	public class PointComponent<T> : MonoBehaviour, IPointComponent where T : PointData
	{
		...
		protected T _data;
		[SerializeField]
		protected PointsPreset<T> _parent;
		...
		void IPointComponent.AssignScriptableIfEmpty()
		{
			if (_data == null)
			{
				...
				_data.name = !string.IsNullOrEmpty(_name) ? _name : _parent.Config.DefaultName;
				_currentName = _data.name;
				string folderPath = _parent.Config.Path + "/" + _parent.Config.FolderName;

				if (!AssetDatabase.IsValidFolder(folderPath))
				{
					AssetDatabase.CreateFolder(_parent.Config.Path, _parent.Config.FolderName);
				}
